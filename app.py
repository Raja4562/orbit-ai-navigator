...
# --- CONFIGURATION ---
# Removed local D:/ path for public GitHub compatibility
...
def get_coords(name_query):
    ...
    try:
        # CRITICAL: Using 'orbit_v42' user agent
        geolocator = Nominatim(user_agent="orbit_v42_public")
        search_query = (
...
if prompt := st.chat_input("Ex: 'Pizza' or 'Anything else?'"):
    ...
    if st.session_state.route_data:
        ...
        if is_next_request and st.session_state.cached_results:
            ...
            with st.spinner("Orbit is judging you..."):
            ...
            with st.spinner("Orbit is thinking..."):
                ...
                if found_places:
                    found_places.sort(
                        key=lambda x: (x["match_score"], x.get("rating", 0)),
                        reverse=True,
                    )
