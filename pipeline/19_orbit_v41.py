...
# --- PAGE CONFIG ---
st.set_page_config(
    layout="wide", page_title="Orbit V41: Truth Core", initial_sidebar_state="expanded"
)
...
def ask_orbit_personality(place_data, user_query, ref_name, dist):
    ...
    # Extract real data to prevent lying
    name = place_data["name"]
    rating = place_data.get("rating", "Unknown")
    vibe = place_data.get("vibes", ["Standard"])[0]
    # Check if address exists in JSON, otherwise tell AI to shut up about it
    address = place_data.get("address", None)

    addr_instruction = (
        f"The address is {address}."
        if address
        else "Do NOT mention specific street names or addresses."
    )

    try:
        sys_prompt = (
            "You are Orbit, a cynical, sarcastic AI navigator. "
            f"The user is at: {ref_name}. "
            f"The place is: {name}. "
            f"Rating: {rating} stars. "
            f"Distance: {dist} miles away relative to {ref_name}. "
            f"{addr_instruction} "
            "Do NOT invent fake addresses like '1234 Lane'. "
            "Keep it under 3 sentences. Be funny."
        )
        full_prompt = (
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs, max_new_tokens=150, temperature=0.7, use_cache=False
            )
        ...
        return f"Found **{name}**. ({dist} miles from {ref_name})."
...
if "history" not in st.session_state:
    st.session_state.history = [
        {"role": "assistant", "content": "Orbit V41. Truth Protocols Active."}
    ]
